...
func createSparkAppConfigMap(app *v1beta2.SparkApplication, submissionID string, createdApplicationId string, kubeClient kubernetes.Interface, driverConfigMapName string) error {
	...
	driverConfigMapData[common.SparkKubernetesDriverPodName] = app.GetNamespace()
...
func buildAltSubmissionCommandArgs(app *v1beta2.SparkApplication, driverPodName string, submissionID string, createdApplicationId string) (string, error) {
	...
	if app.Spec.Image != nil {
		...
		args = args + fmt.Sprintf("%s=%s", common.SparkKubernetesContainerImage, sparkContainerImage) + NewLineString
	}
	if app.Spec.ImagePullPolicy != nil {
		args = args + fmt.Sprintf("%s=%s", common.SparkKubernetesContainerImagePullPolicy, *app.Spec.ImagePullPolicy) + NewLineString
	}
	if len(app.Spec.ImagePullSecrets) > 0 {
		secretNames := strings.Join(app.Spec.ImagePullSecrets, CommaSeparator)
		args = args + fmt.Sprintf("%s=%s", common.SparkKubernetesContainerImagePullSecrets, secretNames) + NewLineString
	}
	if app.Spec.PythonVersion != nil {
		args = args + fmt.Sprintf("%s=%s", common.SparkKubernetesPysparkPythonVersion, *app.Spec.PythonVersion) + NewLineString
	}
	if app.Spec.MemoryOverheadFactor != nil {
		args = args + fmt.Sprintf("%s=%s", common.SparkKubernetesMemoryOverheadFactor, *app.Spec.MemoryOverheadFactor) + NewLineString
	} else {
		args = args + fmt.Sprintf("%s=%s", common.SparkKubernetesMemoryOverheadFactor, "0.1") + NewLineString
	}

	// Operator triggered spark-submit should never wait for App completion
	args = args + fmt.Sprintf("%s=false", common.SparkKubernetesSubmissionWaitAppCompletion) + NewLineString
	...
	if app.Spec.Driver.CoreRequest != nil {
		args = args + fmt.Sprintf("%s=%s", common.SparkKubernetesDriverRequestCores, *app.Spec.Driver.CoreRequest) + NewLineString
	}
	if app.Spec.Driver.CoreLimit != nil {
		args = args + fmt.Sprintf("%s=%s", common.SparkKubernetesDriverLimitCores, *app.Spec.Driver.CoreLimit) + NewLineString
	}
	...
	if app.Spec.Driver.ServiceAccount != nil {
		args = args + fmt.Sprintf("%s=%s", common.SparkKubernetesAuthenticateDriverServiceAccountName, *app.Spec.Driver.ServiceAccount) + NewLineString
	}

	if app.Spec.Driver.JavaOptions != nil {
		driverJavaOptionsList := AddEscapeCharacter(*app.Spec.Driver.JavaOptions)
		args = args + fmt.Sprintf("%s=%s", common.SparkDriverExtraJavaOptions, driverJavaOptionsList) + NewLineString
	}

	if app.Spec.Driver.KubernetesMaster != nil {
		args = args + fmt.Sprintf("%s=%s", common.SparkKubernetesDriverMaster, *app.Spec.Driver.KubernetesMaster) + NewLineString
	}
	...
	if app.Spec.Executor.Image != nil {
		args = args + fmt.Sprintf("%s=%s", common.SparkKubernetesExecutorContainerImage, *app.Spec.Executor.Image) + NewLineString
	}
	...
	if app.Spec.Executor.CoreRequest != nil {
		args = args + fmt.Sprintf("%s=%s", common.SparkKubernetesExecutorRequestCores, *app.Spec.Executor.CoreRequest) + NewLineString
	}
	if app.Spec.Executor.CoreLimit != nil {

		args = args + fmt.Sprintf("%s=%s", common.SparkKubernetesExecutorLimitCores, *app.Spec.Executor.CoreLimit) + NewLineString
	}
	...
	if app.Spec.Executor.ServiceAccount != nil {
		args = args + fmt.Sprintf("%s=%s", common.SparkKubernetesAuthenticateExecutorServiceAccountName, *app.Spec.Executor.ServiceAccount) + NewLineString
	}

	if app.Spec.Executor.DeleteOnTermination != nil {

		args = args + fmt.Sprintf("%s=%t", common.SparkKubernetesExecutorDeleteOnTermination, *app.Spec.Executor.DeleteOnTermination) + NewLineString
	}
	...
	if app.Spec.Executor.JavaOptions != nil {
		args = args + fmt.Sprintf("%s=%s", common.SparkExecutorExtraJavaOptions, *app.Spec.Executor.JavaOptions) + NewLineString
	}
	...
	for key, value := range app.Spec.Executor.EnvVars {
		args = args + fmt.Sprintf("%s%s=%s", common.SparkExecutorEnvVarConfigKeyPrefix, key, value) + NewLineString
	}
	if app.Spec.DynamicAllocation != nil {

		args = args + fmt.Sprintf("%s=true", common.SparkDynamicAllocationEnabled) + NewLineString
		// Turn on shuffle tracking if dynamic allocation is enabled.
		args = args + fmt.Sprintf("%s=true", common.SparkDynamicAllocationShuffleTrackingEnabled) + NewLineString
		dynamicAllocation := app.Spec.DynamicAllocation
		if dynamicAllocation.InitialExecutors != nil {
			args = args + fmt.Sprintf("%s=%d", common.SparkDynamicAllocationInitialExecutors, *dynamicAllocation.InitialExecutors) + NewLineString
		}
		if dynamicAllocation.MinExecutors != nil {
			args = args + fmt.Sprintf("%s=%d", common.SparkDynamicAllocationMinExecutors, *dynamicAllocation.MinExecutors) + NewLineString
		}
		if dynamicAllocation.MaxExecutors != nil {
			args = args + fmt.Sprintf("%s=%d", common.SparkDynamicAllocationMaxExecutors, *dynamicAllocation.MaxExecutors) + NewLineString
		}
		if dynamicAllocation.ShuffleTrackingTimeout != nil {
			args = args + fmt.Sprintf("%s=%d", common.SparkDynamicAllocationShuffleTrackingTimeout, *dynamicAllocation.ShuffleTrackingTimeout) + NewLineString
		}
	}
